...
class Text(object):
    ...
    def __init__(self, string: str, color: tuple[int, int, int], style: str, font_size: int, start: tuple[int, int], width: int, height: int, end_padding: tuple[int, int], fn_ptr_factor: tuple[float, float] = IBM_FNT_PT_FACTOR, additional_colors: dict[str, tuple[int, int, int]] = {}) -> None:
        ...
        self.escape_codes.update({
            CODE_FORMATTING['COLOR_RESET']: self.starting_color, 
            CODE_FORMATTING['STYLE_RESET']: self.starting_style
        })

        if additional_colors: 
            self.escape_codes.update(additional_colors)
        
        self.escape_pattern: re.Pattern = re.compile('(' + TEXT_ESCAPE_CHAR + r'{(?:c|s):[a-z-]+})')
    ...
    def partial_process_atrribs(self, strings: list[str], color: tuple[int, int, int], style: str, pos: list[int], codes: list[str], width: int, height: int) -> list[Section]:
        ...
        for string in strings:
            if len(string) > 0:
                ...
                if len(splitstring) > 1:
                    for part in splitstring:
                        semi_processed.append(Section(part, color, style, tuple(pos)))

                        pos = [self.start[0], pos[1] + height]

                    pos[1] -= height   
                else:
                    semi_processed.append(Section(splitstring[0], color, style, tuple(pos)))
            ...
            if codes:
                code = codes.pop(0)
                if code[2] == 'c':
                    t = self.escape_codes.get(code, color)
                    assert isinstance(t, tuple)
                    color = t
                else:
                    t = self.escape_codes.get(code, style)
